...
df = pd.read_csv('~/programming/python/uni/projects/market_risk/dummy_data.csv').dropna().reset_index().drop('index', axis=1).iloc[:300]
...
default_prob = estimate_default_prob(df=df, threshold=0.1, num_samples=1, window_size=252, step=1, score='t_score')

# Print default probabilities
print(default_prob)

#ToDo write a loop for all funds
