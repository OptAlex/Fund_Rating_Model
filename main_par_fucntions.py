...
def calculate_default_probabilities_wrapper(sim_results, thresholds, sim_number):
    ...
    df_default_probabilities = calculate_default_probabilities(threshold_results_dict, sim_number)
...
if __name__ == "__main__":
    ...
    for i in range(NUM_BOOTSTRAPPING):
        ...
        sim_results = pool.starmap(run_simulation, [(x, bootstrapped_returns) for x in range(SIM_NUMBER)])
        ...
        df_default_probabilities = calculate_default_probabilities_wrapper(sim_results, THRESHOLDS_LOG, SIM_NUMBER)
        ...
        for i, sim_result in enumerate(sim_results):
            ...
            df_all_weighted_avg_log_returns = pd.concat([df_all_weighted_avg_log_returns, df_weighted_avg_log_returns],
                                                        axis=1)
            ...
            df_all_weighted_avg_standard_returns = pd.concat([df_all_weighted_avg_standard_returns, df_weighted_avg_standard_returns],
                                                        axis=1)
            df_all_weighted_avg_standard_returns.rename(columns={0: sim_name}, inplace=True)
        ...
        df_all_default_prob_port = pd.concat([df_all_default_prob_port, df_weighted_avg_log_default_prob], axis=1)
